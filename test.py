...
if __name__ == '__main__':
    beam_size = 5
    boxes = torch.load("frisbee_boxes.pt")
    prediction, best_sequence, best_attentions = generate_caption(boxes, beam_size)
    torch.save(best_attentions,"frisbee_attentions.pt", _use_new_zipfile_serialization=False)
    torch.save(best_sequence, "frisbee_sequence.pt", _use_new_zipfile_serialization=False)
    print(prediction)